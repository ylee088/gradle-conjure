...
public class ConjureProductDependenciesExtension {
    ...
    public static final String ENDPOINT_VERSIONS_MANIFEST_KEY = "Endpoint-Versions";

    private final Set<ServiceDependency> productDependencies = new HashSet<>();
    private final SetProperty<EndpointVersion> endpointVersions;
    private final ProviderFactory providerFactory;

    @Inject
    public ConjureProductDependenciesExtension(Project project) {
        this.endpointVersions =
                project.getObjects().setProperty(EndpointVersion.class).empty();
    ...
    public final void endpointVersion(@DelegatesTo(EndpointVersion.class) Closure<?> closure) {
        endpointVersions.add(providerFactory.provider(() -> {
            EndpointVersion emv = new EndpointVersion();
            ConfigureUtil.configureUsing(closure).execute(emv);
            return emv;
        }));
    }

    public final SetProperty<EndpointVersion> getEndpointVersions() {
